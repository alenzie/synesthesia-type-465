...
- **Fixtures generated, not downloaded**: `tools/make-wavetable-fixtures.js` writes `tools/fixtures/*.wav|.wt`
  (a `clm` 4-frame 2048 table, a no-clm 2048×2, a stereo X/Y table — circle→star morph, int16+int24+float32
  variants, a Surge `.wt`, a corrupt header, an odd-tail file, a 300-frame file). Deterministic (no RNG), so
  fixtures are reproducible and can be gitignored except the generator.
- **Canonical hash (the port-spec definition, used everywhere):** SHA-256 (small sync JS implementation —
  ...
  resolves.
- **Fixture matrix (generated by `tools/make-wavetable-fixtures.js`, deterministic):** clm 4×2048; no-clm
