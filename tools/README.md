...
| `wavetable-check.js [html]` | Wavetable C1–C3 core: the RIFF/`clm`/Surge parser over a 26-file hostile fixture matrix, the canonical content hash (against an independent node-crypto oracle), the FFT mip pyramid (band-limiting, DC/Nyquist removal, amplitude invariance across levels, determinism — all judged by a naive-DFT oracle, never by `wtFFT` itself), the worker source, and the oscillator (dedicated phase, frame mapping, LOD, availability fallback, anti-alias payoff). |
| `wt-ui-check.js [url] [--chromium\|--firefox]` | The wavetable import UI, driven through the REAL file input in real browsers (both by default): the row appears for the generator, mono/stereo/Surge fixtures import and become the active table, the imported table actually produces audio, a corrupt file reports readable text without disturbing the live table, and a no-clm file offers the frame-size chooser. |
| `engine-stability-check.js [html]` | Sweeps cutoff × resonance × sample-rate × LFO (420 combinations) asserting the state-variable filter never diverges, that the clamp satisfies the Chamberlin pole condition at every resonance, and that resonance and the clamped high cutoff still audibly do something. Born from the LFO→CUTOFF screech. |
