...
2. Level `k` has frame length `Nk = N >> k`, for k = 0 … log2(N/8) (deepest frame = 8 samples). Retain
   ...
   so).
3. At playback derive a **fractional LOD** from the sampler's actual per-sample phase increment `inc`
   (cycles/sample): `lod = clamp(log2(max(1, frameSize * inc)), 0, L-1)`; render the two adjacent integer
   levels and crossfade by the fraction (no hard switching, so no hysteresis machinery — one smoothed
   per-voice `wtLod` only if measurement shows jitter).
...
- **One table-availability state machine (all hosts, no contradictions):**
  ...
  the rule everywhere.
- **Worklet transport:** ship the mip pyramid as **copied buffers posted as transferables** in a dedicated
  message, *not* through `_pushPatch`'s JSON (exact message shape: build plan C3). The patch carries only
  the hash; the processor's cache fills from table messages, and a patch referencing a not-yet-arrived hash
  puts the generator in `pending` — the state machine above governs (BUILT-IN renders; never silence).
- Ship **ONE built-in table** (`builtin:basic`, 16×2048 basic-shapes morph, generated by code in every
  core) — the pre-import experience, the harness fixture, and the missing-table fallback in one object.
